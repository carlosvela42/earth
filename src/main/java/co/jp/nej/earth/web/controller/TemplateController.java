...
public class TemplateController extends BaseController {
    ...
    public String showList(String templateType, Model model, HttpServletRequest request) throws EarthException {
        ...
        UserInfo userInfo = ConversionUtil.castObject(session.getAttribute(Session.USER_INFO), UserInfo.class);
        ClientSearchForm searchForm = new ClientSearchForm();
        if (!EStringUtil.isEmpty(templateType) && !EStringUtil.isEmpty(workspaceId)) {
//            mgrTemplates = templateService.getTemplateByType(workspaceId, templateType, userInfo.getUserId());
            mgrTemplates = templateService.getTemplateByType(workspaceId, templateType);

            String keyScreen = getKeyScreen(templateType);
            if(!EStringUtil.isEmpty(keyScreen)){
                SessionUtil.clearAllOtherSearchCondition(session, keyScreen);
                searchForm = (ClientSearchForm) SessionUtil.getSearchCondtionValue(session,keyScreen);
                if(searchForm == null) {
    ...
    public String addNew(Model model, HttpServletRequest request, ClientSearchForm searchForm) throws EarthException {
        ...
        String keyScreen = getKeyScreen(templateType);
        if(!EStringUtil.isEmpty(keyScreen)){
            SessionUtil.setSearchCondtionValue(request.getSession(), keyScreen, searchForm);
        }
        templateForm.setTemplateId(eTemplateId.getAutoId(workspaceId));
        templateForm
            .setTemplateTableName(eTemplateTableName.getTemplateTableName(templateType
    ...
    public String showDetail(@ModelAttribute("templateIds") String templateId, Model model, HttpServletRequest request,
            ClientSearchForm searchForm) throws EarthException, JsonParseException, JsonMappingException, IOException {
        TemplateKey templateKey = new TemplateKey();
        String workspaceId = SessionUtil.getSearchConditionWorkspaceId(request.getSession());
        if (!EStringUtil.isEmpty(templateId) && !EStringUtil.isEmpty(workspaceId)) {
            String templateType = request.getSession().getAttribute("templateType").toString();
            String keyScreen = getKeyScreen(templateType);
            if(!EStringUtil.isEmpty(keyScreen)){
                SessionUtil.setSearchCondtionValue(request.getSession(), keyScreen, searchForm);
            }
   ...
   */
    ...
    private String getKeyScreen(String templateType) {
        ...
        switch(templateType) {
            case "WORKITEM":
                keyScreen = Constant.ScreenKey.WORKITEM_TEMPLATE;
                break;
            case "FOLDERITEM":
                keyScreen = Constant.ScreenKey.FOLDERITEM_TEMPLATE;
                break;
            case "DOCUMENT":
                keyScreen = Constant.ScreenKey.DOCUMENT_TEMPLATE;
                break;
            case "LAYER":
                keyScreen = Constant.ScreenKey.LAYER_TEMPLATE;
                break;
            case "PROCESS":
                keyScreen = Constant.ScreenKey.PROCESS_TEMPLATE;
                break;
            default:
        }
        return keyScreen;
