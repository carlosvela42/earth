...
public class WorkItemDaoImpl extends BaseDaoImpl<WorkItem> implements WorkItemDao {
    ...
    // Template wkiTemplate = templateDao.getTemplates("",
    // wkiTemplateIds).get(0);
    //
    // // 3. Get WorkItem Data by Template.
    // QTemplateData qTemplateData = QTemplateData.newInstance("QTemplateData",
    // "spring_boot_test", wkiTemplate);
    // QBean<TemplateData> selectList = Projections.bean(TemplateData.class,
    // qTemplateData.all());
    // TemplateData templateData =
    // queryFactory.getDataTableByTemplate(selectList, wkiTemplate,
    // qTemplateData , null);
    //
    ...
    // List<Document> fDocuments = new
    // ArrayList<Document>(folderItem.getDocuments());
    ...
    // Map<Long, TemplateData> docTemplateDatas = new HashMap<Long,
    // TemplateData>();
    // for (Template docTemplate : docTemplates) {
    // QTemplateData qDocTemplateData =
    // QTemplateData.newInstance("QTemplateData", "spring_boot_test",
    // docTemplate);
    // QBean<TemplateData> docSelectList = Projections.bean(TemplateData.class,
    // qDocTemplateData.all());
    // TemplateData docTemplateData =
    // queryFactory.getDataTableByTemplate(docSelectList, docTemplate,
    // qDocTemplateData , null);
    ...
    //
    // public WorkItem getWorkItemStructure(long workItemId) {
    // QWorkItem qWorkItem = QWorkItem.newInstance("QWorkItem",
    // "spring_boot_test", "WORKITEM");
    // QFolder qFolder = QFolder.newInstance("QFolder", "spring_boot_test",
    // "Folder");
    // QFolderItem qFolderItem = QFolderItem.newInstance("QFolderItem",
    // "spring_boot_test", "FOLDERITEM");
    // QDocument qDocument = QDocument.newInstance("QDocument",
    // "spring_boot_test", "DOCUMENT");
    // QLayer qLayer = QLayer.newInstance("QLayer", "spring_boot_test",
    // "LAYER");
    // QTemplate qTemplate = QTemplate.newInstance("QTemplate",
    // "spring_boot_test", "TEMPLATE");
    //// QField qField = QField.newInstance("QFsield", "spring_boot_test",
    // "FIELD");
    ...
    // .innerJoin(qTemplate).on(qWorkItem.templateId.eq(qTemplate.templateId)
    // .or(qDocument.templateId.eq(qTemplate.templateId)))
    // .where(qWorkItem.workitemId.eq(workItemId))
    // .orderBy(qWorkItem.workitemId.asc(), qFolder.folderId.asc(),
    // qFolderItem.folderItemId.asc(), qDocument.documentId.asc(),
    // qLayer.layerId.asc())
    // .getResults();
    //
    ...
    // // Get FolderItem info.
    // Long folderItemId =
    // resultSet.getLong(FolderItemColumnName.FOLDER_ITEM_ID.columnName);
    // Long documentId =
    // resultSet.getLong(DocumentColumnName.DOCUMENT_ID.columnName);
    // Long docTemplateId = resultSet.getLong("document_template_id");
    // if (folderItem.getFolderItemId() != null &&
    // folderItem.getFolderItemId().longValue() != folderItemId.longValue()) {
    // folderItem.addDocument(document);
    // workItem.addFolderItem(folderItem);
    // folderItem = new FolderItem();
    // document = new Document();
    // } else if (document.getId() != null && document.getId().longValue() !=
