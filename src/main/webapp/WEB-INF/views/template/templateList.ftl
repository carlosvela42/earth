...
window.onload = function() {
    ...
    function filter() {
        ...
        templateNameInput = document.getElementById('templateNameInput').value
                .toUpperCase();
        templateTableNameInput = document.getElementById('templateTableNameInput').value
        .toUpperCase();

        // Loop through all list items, and hide those who don't match the search query
        for (i = 0; i < $("#templateTable tr ").length - 2; i++) {

            if (($("#templateId" + i).html().toUpperCase()
                    .indexOf(templateIdInput) > -1)
                    && ($("#templateName" + i).html().toUpperCase().indexOf(
                            templateNameInput) > -1)
                     && ($("#templateTableName" + i).html().toUpperCase().indexOf(
                            templateTableNameInput) > -1)       ) {
                $("#row" + i).show();
...
<form object="mgrTemplates" method="post" id="mgrTemplateForm"
   ...
   <table id="button">
        <tr>
            <td><a href="${rc.getContextPath()}/template/addNew"
                class="button">新規</a></td>
            <td><input type="button" class="button" value="編集"
                onclick=""></td>
    ...
    <input type="hidden" id="workspaceId" name="workspaceId" value="0"> 
    <div>
    ...
    <table border="1" id="templateTable">
            ...
            <tr>
                <td colspan="2"><input type="text" id="templateIdInput" onkeyup="filter()"
                 placeholder="Search for ID.."></td>
                <td><input type="text" id="templateNameInput" onkeyup="filter()"
                 placeholder="Search for name.."> </td>
                <td><input type="text"id="templateTableNameInput" onkeyup="filter()" 
                placeholder="Search for table names.."></td>
            </tr>
            <#if mgrTemplates??>
