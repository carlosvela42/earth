...
window.onload = function() {
    ...
    function filter() {
        ...
        for (i = 0; i < $("#sitesTable tr ").length - 2; i++) {
            ...
            if (($("#siteId" + i).html().toUpperCase()
                    .indexOf(siteIdInput) > -1)) {
                $("#row" + i).show();
...
<form object="sites" method="post" id="sitesForm"
    ...
    <table border="1" id="sitesTable">
            ...
            <tr>
                <td colspan="3"><input type="text" id="siteIdInput" onkeyup="filter()"
                 placeholder="Search for ID.."></td>
            </tr>
            <#if sites??>
                <#list sites as site>
                <tr id="row${site?index}">
                     <td><input type="checkbox" id="delRow${site?index}"name="DeleteRow" value="${site.siteId}"></td>
                     <td><a href="${rc.getContextPath()}/site/showDetail?siteId=${site.siteId}"  class="button">編集</a></td>
                     <td id="siteId${site?index}">${site.siteId}</td>
                </tr>
                </#list>
            </#if>
        </table>
        ...
        <tr>
            <td><input type="button" class="button" value="削除"
                onclick="validate()"></td>
            <td><input type="checkbox" name="deleteSite"
                value="deleteSite">Confirm Delete</td>
        </tr>
    </table>
</form>
