...
IV.prototype.mousedown = function () {
	var m = d3.mouse(this);
	imageViewer.i = parseInt(imageViewer.i);
	imageViewer.x = m[0];
	imageViewer.y = m[1];
	var x = m[0];
	var y = m[1];
	...
	if ($("#testLine").prop("checked")) {
		...
		anno = vis.append("line").attr("x1", x).attr("y1", y).attr("x2", x)
				.attr("y2", y).attr("id", imageViewer.i).attr("stroke", penColor).attr(
						"stroke-width", scale).attr("class", layerActive).on(
								"click", selectClick);
		vis.on("mousemove", imageViewer.mousemove);
	}
	...
	if ($("#rectangle").prop("checked")) {
		...
		anno = vis.append("rect").attr("x", x).attr("y", y).attr("width", 0)
				.attr("height", 0).attr("id", imageViewer.i).attr("fill", fillColor).attr(
	...
	if ($("#text").prop("checked")) {
		...
		anno = vis.append("foreignObject").attr("x", x).attr("y", y).attr(
				"width", 0).attr("height", 0).attr("id", imageViewer.i).attr("class",
				"text " + layerActive).on("click", selectClick);
		$('#' + imageViewer.i).append('<textarea id="t' + imageViewer.i + '"/>');
		$("#t" + imageViewer.i).css("color", penColor)
				.css("backgroundColor", "transparent")
.css("border", "transparent")
.css("font-family", "Times New Roman")
.css("fontSize", 8)
.css("font-style", "normal").css("width", 0)
.css("height", 0);
		vis.on("mousemove", imageViewer.mousemove);
	}
	if ($("#highlight").prop("checked")) {
		...
		anno = vis.append("rect").attr("x", x).attr("y", y).attr("width", 0)
				.attr("height", 0).attr("fill", highlightColor).attr("stroke",
						...
						layerActive).on("click", selectClick).attr("id", imageViewer.i);
		vis.on("mousemove", imageViewer.mousemove);
	}
	if ($("#comment").prop("checked")) {
		...
		$('#' + imageViewer.i).append(
				'<img id="i' + imageViewer.i
						+ '" src="resources/images/imageViewer/comment.jpg"/>');
		var userInfo = $('#userInfo').text();
		$('#' + imageViewer.i).append(
				'<textarea id="t' + imageViewer.i + '">' + userInfo.slice(0,userInfo.indexOf("/")) + '</textarea>');
		$("#t" + imageViewer.i).css("width", 0);
		$("#t" + imageViewer.i).css("height", 0);
		$("#i" + imageViewer.i).css("width", 0);
		$("#i" + imageViewer.i).css("height", 0);
		vis.on("mousemove", imageViewer.mousemove);
	}
...
IV.prototype.mousemove = function() {
	var m = d3.mouse(this);
	var x = imageViewer.x;
	var y = imageViewer.y;
	if (0 <= m[0] <= $('#svg').attr('width')
		...
		if ($("#text").prop("checked")) {
			...
			anno.attr("width", Math.abs(m[0] - x));
			anno.attr("height", Math.abs(m[1] - y));
			$("#t" + imageViewer.i).css("width", Math.abs(m[0] - x));
			$("#t" + imageViewer.i).css("height", Math.abs(m[1] - y));
		}
		if ($("#comment").prop("checked")) {
			if (m[1] - y < 0) {
				anno.attr("y", m[1]);
			...
			anno.attr("height", Math.abs(m[1] - y));
			$("#t" + imageViewer.i).css("width", Math.abs(m[0] - x));
			$("#t" + imageViewer.i).css("height", Math.abs(m[1] - y) / 2);
			$("#i" + imageViewer.i).css("width", Math.abs(m[0] - x));
			$("#i" + imageViewer.i).css("height", Math.abs(m[1] - y) / 2);
		}
	}
}

IV.prototype.mouseup = function() {
	var vis = imageViewer.vis;
	var scale = imageViewer.scale;
