...
public class LoginControllerTest extends BaseTest {
    ...
    public void testLogin2() throws EarthException{
        List<Message> listMessage = userService.login("", "123456", session);
        Assert.assertTrue(listMessage != null && listMessage.size() > 0 && listMessage.get(0).getCode().equals(MessageCodeLogin.USR_BLANK));
    }
    
    @Test
    public void testLogin3() throws EarthException{
        List<Message> listMessage = userService.login("admin", "", session);
        Assert.assertTrue(listMessage != null && listMessage.size() > 0 && listMessage.get(0).getCode().equals(MessageCodeLogin.PWD_BLANK));
    }
    
    @Test
    public void testLogin4() throws EarthException{
        List<Message> listMessage = userService.login("", "", session);
        Assert.assertTrue(listMessage != null && listMessage.size() == 2);
    }
    
    @Test
    public void testLogin5() throws EarthException{
        List<Message> listMessage = userService.login("admin", "123457", session);
        Assert.assertTrue(listMessage != null && listMessage.size() > 0 && listMessage.get(0).getCode().equals(MessageCodeLogin.INVALID_LOGIN));
    }
